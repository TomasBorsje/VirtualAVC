...
int whiteLineLoop(int TURN_MULTIPLIER, int ROBOT_SPEED,ImagePPM currentFrame,double vLeft,double vRight){
...
void emergencyStuckManuveours(){
	std::cout<<"robot is stuck"<<std::endl;
	setMotors(-20.0,-20.0);
	usleep(500000);
	setMotors(-5.0,5.0);
	while(isLeftSideRed(cameraView)==false){
		takePicture();
		usleep(17500);
	}
}
...
void stuckFunction(){
	...
	OpenPPMFile("view1.ppm", currentFrame);
	
	for(int i = 0;i<currentFrame.width-1;i=i+5){
		for(int j = 0;j<currentFrame.height-1;j=j+5){
			vec1.push_back(get_pixel(currentFrame,j,i,1));
		}
	}
	OpenPPMFile("view2.ppm", currentFrame);
	for(int i = 0;i<currentFrame.width-1;i=i+5){
		for(int j = 0;j<currentFrame.height-1;j=j+5){
			vec2.push_back(get_pixel(currentFrame,j,i,1));
		}
	}
	...
	if(matchescounter>vec1.size()*0.4){
		std::cout<<"robot is stuck------------------------------------------"<<std::endl;
		emergencyStuckManuveours();
	}
}
...
int main(){
    ...
    const static float TURN_RATIO = 0.4;
	...
	bool whatimage = true; //code needs two images to check if the robot gets stuck. this will be used to alternate the image being used
	int dostuckcheck;
    // Variable to store current frame in
    ImagePPM currentFrame; 
    
    int whitepixels;
    do{//this loop follows the white line movement style until it runs out of white line. then the loop finishes and its inside the red maze so the main loop can run
		// Take picture
		takePicture(); 
	    // Save it as view.ppm
		SavePPMFile("view1.ppm",cameraView); 
		// Open view.ppm, and save the contents in currentFrame variable
		OpenPPMFile("view1.ppm", currentFrame); 
		whitepixels = whiteLineLoop(TURN_MULTIPLIER, ROBOT_SPEED,currentFrame,vLeft,vRight);
		}while(whitepixels>0);
    ...
    while(1){
		...
		takePicture(); 
	    // Save it as view.ppm
	    std::cout<<"whatimage: "<<whatimage<<std::endl;
	    if(whatimage){
		SavePPMFile("view1.ppm",cameraView); 
		}
		else{
			SavePPMFile("view2.ppm",cameraView); 
		}
		// Open view.ppm, and save the contents in currentFrame variable
		if(whatimage){
		OpenPPMFile("view1.ppm", currentFrame); 
		...
		middleRedPixels = 0;
			...
			if(isPixelRed(center1) || isPixelRed(center2)) // Is there a barricade RIGHT infront of us?
			{
				if(isLeftSideRed(currentFrame)==true)
				{
					...
					std::cout<<"Barricade RIGHT infront of us, reversing right on the spot."<<std::endl;
				}
				else
				{
					// Turn left while reversing
					setMotors(-ROBOT_SPEED, ROBOT_SPEED); // One of the center pixels was red, there's a barricade infront of us
					std::cout<<"Barricade RIGHT infront of us, reversing left on the spot."<<std::endl;					
				}
			}
			else if(isLeftSideRed(currentFrame)==true)
			{
				...
				{
					setMotors(ROBOT_SPEED, ROBOT_SPEED*TURN_RATIO); // Turn right
					std::cout<<"Wall left, barricade infront, turning right."<<std::endl;
				}
				else
				{
					setMotors(ROBOT_SPEED, ROBOT_SPEED); // Continue forward
					std::cout<<"Wall left, going forward."<<std::endl;
				}
			}
			else // There is no wall to the left of us
			{
				setMotors(ROBOT_SPEED*TURN_RATIO, ROBOT_SPEED); // Turn left
				std::cout<<"No wall left, turning left."<<std::endl;
			}
		dostuckcheck = rand() % 5 + 1;
		if(dostuckcheck == 1){
			std::cout<<"checking if stuck+++++++++++++++++++++++++++++++++++++++++++++++++++++"<<std::endl;
			stuckFunction();//function both checks if robot is stuck and responds if it is
		}
		whatimage = !whatimage;//changes whatimage for next loop repetition
